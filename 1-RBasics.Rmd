...
```{r}
paste("To be", "or not", "to be,", "That is the question!")
```

(@) Can you explain what happened in 2-3 sentences? 

> The paste function tells the computer to take the strings that are seperated by a comma (i.e., the separator) and join these individual strings into a single string. The computer then just takes the data from the separate strings and creates an output of a single string that contains all of the separate bits of data by taking whatever it finds in those parentheses that is in quotation marks and joining it all together into a single set of quotation marks. Without the separator (the comma), that function would not work.
...
> Each of the above lines tests the statement being made against the values on either side of the operators.
...
> The code multiplies each number in the vector by 2 and shows the results for each item. This is because the variable my.numbers contains all these values, so since no specific value from the vector was specified, the computer applies the operation to all the numbers. Vectors, then, allow the programmer to apply an operation to multiple different values or strings simultaneously.
...
> The code only multiplied the third item of the vector, 6, by two. It did this because the item was specified by the '[3]'. Any item in the vector can be called by using the brackets.

(@) You try, create a list of five items and store it in a descriptive variable. 
```{r}
cheeseburger <- c("lettuce", "tomato", "ketchup", "mayo", "American cheese", "beef patty", "bun")
```
...
In real life, typically you wouldn't want to store this code in your script file. You probably don't need to pull up the documentation for the function every time you run that piece of code. But for the purposes of this worksheet we're adding it to our `.Rmd` document. 

(@) Use the console to find the documentation for `floor()`? Try it and then tell me, what does that function do? 

> According to the manual, the floor() function "takes a single numeric argument x and returns a numeric vector containing the largest integers not greater than the corresponding elements of x."
...
> CLolumn z takes the value from the first column, multiplies it by 2 and adds the value in column y to it.
...
>This information in this dataset spans from 1965 through 1985.
...
> I used the manual to better understand the function and then made my modification.
...
>Here, gayguides is the name of the dataset, the dollar sign ($) tells the computer to look for a specific column in that dataset and title is the name of the column the computer is looking for while the [500] specifies the specific line that the computer will look for.
...
> There were 271 obs. in the charlestonManumissions dataset.
...
> Looking at the charlestonManumissions dataset, I learned several things about all of the manumissions filed in Charleston District from 1776-1800 that were contained in the Miscellaneous Records section of the South Carolina Department of Archives and History. 

>Based on this dataset I was quickly able to ascertain that there were only 271 slaves on record as being freed legally by manumission in Charleston in the four years between 1776-1800. This would average out to about 68 slaves freed by manumission per year during this time. It might be interesting to compare this number to the estimated total number of slaves that lived in the area in these same years. 

>By using the table() function I was able to discover that 145 of these slaves were manumised for solely moral reasons.This makes up almost 54 percent of all manumissions given during this period. I also used the table() function to look at the names, and I discovered that it appears as though there were far more female slaves manumised than male. It would be interesting to get the count to check this and get a percentage. However, I am not sure how you would best accomplish that with this dataset as there is no column for gender.

>One thing I tried but couldn't figure out was to count the number of occurences of a specific year in the date column. Since the date column has day month year, I couldn't figure out how to get the table function to show just the number of occurences for only the year.

>Lastly, there were interesting notes in the dataframe that gave lots of detail that lends context to the meaning of the "motives" in the frame. This could be used to help categorize the different conditions under each heading that received manumission.
