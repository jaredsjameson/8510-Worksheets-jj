...
```{r}
y <- c(7.7, 77.7, 777.77, 777777, 7, 17, 57)
...
> In the above example the negative "c" told the computer to leave out the variables listed in the parentheses when printing the variables from the vector.
...
> In the first line, for each variable that that the command marks as false, the computer does not print that variable. In the second line, only variables that are greater than 7 are printed.
...
>So, here is what I think is going on: The first command tells the computer to use "letters" to assign a name to each variable in the vector up to the forth item ([1:4]) and to assign that set of variable names to a new variable. So, the new variabl represents all that information and the second command prints one specific variable from the named set, in this case it was the variable named "d" that equals 8.35.
...
```{r}
matrix(c(2, 4, 6, 8, 10, 12, 14, 16, 18), byrow = TRUE, nrow = 3)
```
...
> It is different because gayguides is a dataframe, and it contains more information than the earlier matrix. The dataframe appears to just be a more organized matrix that give information like number of rows, number of columns, column headers while also adding styling like cell shading, and borders. You also even get an arrow at the top to help you move through the data in the columns and rows.
...
cols <- c(1, 3, 8)
charlestonManumissions[,cols]
                       

```

> >I chose the manumissions dataset, and I chose to subset the Name, Owner, Motive, and Notes column. The reason for this was I wanted to see for one if there were clear relations between the types of morals that were applied that changed according to gender. This is difficult because the names aren't classified by gender, and I was hoping this might give me an idea as to how I could try and sort the names by gender without going through every entry.
...
>As far as I can tell, the above code just tells the computer to get the city column in the gayguides dataset, and then to print the results from row 100 to 110.
...
if(length(presidents) > 5) {
  "There are more than five presidents on this list"
} else if(length(presidents) < 5) {
  "There are less than five presidents on this list"
}
```
...
if(max(gayguides$Year) > 1990) {
  print("This list has years after 1990")
} else { 
  print("There are no entries for years after 1990 in this data")
  }
...
> Well, to build this code, it was just a matter of first being able to find the correct function to calculate the highest value in the Year column of the dataframe. That was the "max" function. After learning how to implement that, it was just a matter of adding it to the "if" function at that point. 
...
> This works because the length function returns the number of variables contained in the vector. By writing pring(1:length(presidents) it tells the computer to print the entire column up to the last entry.
...
```{r}
myclasses <- c("RCID 8800: Studios 1 & 2", "History 8510: Digital History")
...
>This loop only prints 3 and 4 because that is what it is coded to do. First the for loop skips the values that are less than 3 and goes to the next part of the loop, then it stops as soon as the loop reaches the 5 item in the list. This leaves only 3 and 4 as available values for the loop to print.
...
```{r}
historyStudents("Johnny", "English 1030")
```
...
```{r}
charcount <- function(charstr) {
  nchar(charstr)
  statement <- paste("There are", nchar(charstr), "characters in the string:", charstr, "!", sept="")
  statement
}
...
> I just looked at what each individual portion of the function was doing and then substituted the things that I wanted it to do in each of those locations.

The body of a function can use any of the techniques we've learned in this worksheet like loops and if statements. For example: 
```{r}
grade <- function(x) {
  if (x > 90) {
  "A"
...
>His name doesn't get printed out because I assigned his grade to his name. Now if I run his name, the grade will  be printed.
...
voterbyoccupation <- function(occupation) {
data(BostonWomenVoters)  

i <- 1
  
for (i in 1:length(BostonWomenVoters$Occuptation[i])) {
    if (BostonWomenVoters$Occupation[i] == occupation) {
     data.frame(BostonWomenVoters[i])
